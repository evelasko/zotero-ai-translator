...
exports.BrowserContentExtractor = void 0;
...
class BrowserContentExtractor {
    ...
    async extractFromSourceText(sourceText) {
        if (this.config.debug) {
            console.log(`[BrowserContentExtractor] Extracting content from source text (${sourceText.length} chars)`);
        }
    ...
    async fetchWithRetry(url, attempt = 1) {
        try {
            ...
            if (!response.ok) {
                throw new types_1.UrlFetchError(`HTTP ${response.status}: ${response.statusText}`, response.status);
            }
            return response;
        }
        catch (error) {
            if (attempt < this.config.maxRetries) {
                if (this.config.debug) {
                    console.log(`[BrowserContentExtractor] Retry ${attempt}/${this.config.maxRetries} for URL: ${url}`);
                }
     ...
     */
    extractReadableContent(document) {
        // Remove script and style elements
        const scriptsAndStyles = document.querySelectorAll('script, style, nav, header, footer, aside, .sidebar, .menu, .navigation');
        ...
        try {
            ...
            try {
                ...
                if (pdfMetadata.info) {
                    ...
                    const info = pdfMetadata.info; // PDF.js metadata has dynamic properties
                    if (info.Author) {
...
exports.BrowserContentExtractor = BrowserContentExtractor;
