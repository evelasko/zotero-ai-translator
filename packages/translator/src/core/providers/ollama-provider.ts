...
export class OllamaProvider implements LLMProvider {
  ...
  createClassificationModel(config: AIProviderConfig): BaseChatModel {
    if (config.provider !== 'ollama') {
      throw new ConfigurationError('Invalid provider configuration for Ollama');
    }
  ...
  validateConfig(config: AIProviderConfig): void {
    ...
    if (ollamaConfig.classificationModel && !this.isValidModel(ollamaConfig.classificationModel)) {
      throw new ConfigurationError(
        `Invalid Ollama classification model: ${ollamaConfig.classificationModel}`,
      );
    }

    if (ollamaConfig.extractionModel && !this.isValidModel(ollamaConfig.extractionModel)) {
      throw new ConfigurationError(
        `Invalid Ollama extraction model: ${ollamaConfig.extractionModel}`,
      );
    }

    if (
      ollamaConfig.temperature !== undefined &&
    ...
    if (ollamaConfig.baseUrl) {
      try {
        ...
        throw new ConfigurationError(`Invalid Ollama base URL: ${ollamaConfig.baseUrl}`);
      }
    }
    ...
    if (ollamaConfig.requestOptions) {
      ...
      if (numThread !== undefined && (numThread <= 0 || !Number.isInteger(numThread))) {
        throw new ConfigurationError('Number of threads must be a positive integer');
      }

      if (numGpu !== undefined && (numGpu < 0 || !Number.isInteger(numGpu))) {
        throw new ConfigurationError('Number of GPU layers must be a non-negative integer');
      }
    }
  }
  ...
  getModelCapabilities(modelName: string): ModelCapabilities {
    const ollamaModel = modelName as OllamaModel;
   ...
   */
  isAvailable(): boolean {
    return ProviderDetector.isProviderInstalled('ollama');
  }
  ...
  private isValidModel(model: string): boolean {
    ...
    return validModels.includes(model as OllamaModel);
  }

  /**
   * Check if a model supports multimodal input
   */
  private isMultimodalModel(model: OllamaModel): boolean {
  ...
  static getRecommendedModels() {
    return {
      classification: 'llama3.1:8b' as OllamaModel,
      extraction: 'llama3.1:8b' as OllamaModel,
      highAccuracy: 'llama3.1:70b' as OllamaModel,
      costEffective: 'llama3.2:3b' as OllamaModel,
      coding: 'codellama:13b' as OllamaModel,
      multimodal: 'llama3.2' as OllamaModel,
      lightweight: 'phi3:mini' as OllamaModel,
    };
  }
